...
import {fetchAddListing} from "../api/listing-add";
...
export const addListing = (listing: IListingData) : AppThunkAction => {
    return (dispatch: AppDispatch) => {
        dispatch({type: ADD_LISTING_REQUEST});
        fetchAddListing(listing)
            .then(res => {
                if(res.success){
                    dispatch({type: ADD_LISTING_SUCCESS, payload: res.listing})
                } else {
                    dispatch({type: ADD_LISTING_FAILURE, payload: res.message})
                }
            })
            .catch((err: any) => {
                dispatch({type: ADD_LISTING_FAILURE, payload: err})
            })
    }
}